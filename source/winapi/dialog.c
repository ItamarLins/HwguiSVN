/*
 * $Id: dialog.c 3907 2026-09-07 02:52:58Z itamarlins $
 *
...
/*=============================================================================
 * hwg_tab_is_disabled()
 * Returns TRUE if tab is marked disabled
 *===========================================================================*/
...
/*=============================================================================
 * hwg_draw_tab_item()
 * Draws a tab item (owner-draw support)
 *===========================================================================*/
...
/*=============================================================================
 * HWG_GETDLGITEM()
 * Gets a dialog item handle
 *===========================================================================*/
HB_FUNC( HWG_GETDLGITEM )
{
   HWND hWnd = GetDlgItem( ( HWND ) HB_PARHANDLE( 1 ), hb_parni( 2 ) );
   HB_RETHANDLE( hWnd );
}

/*=============================================================================
 * HWG_GETDLGCTRLID()
 * Gets a dialog control ID
 *===========================================================================*/
...
HB_FUNC( HWG_SETDLGITEMTEXT )
{
   ...
   SetDlgItemText( ( HWND ) HB_PARHANDLE( 1 ), hb_parni( 2 ),
                   HB_PARSTR( 3, &hText, NULL ) );
   hb_strfree( hText );
}

/*=============================================================================
 * HWG_SETDLGITEMINT()
 * Sets dialog item integer value
 *===========================================================================*/
HB_FUNC( HWG_SETDLGITEMINT )
{
   SetDlgItemInt( ( HWND ) HB_PARHANDLE( 1 ), hb_parni( 2 ),
                  ( UINT ) hb_parni( 3 ),
...
HB_FUNC( HWG_GETDLGITEMTEXT )
{
   ...
   GetDlgItemText( ( HWND ) HB_PARHANDLE( 1 ), hb_parni( 2 ),
                   lpText, uiLen );
   HB_RETSTR( lpText );
   hb_xfree( lpText );
}
...
/*=============================================================================
 * HWG_GETEDITTEXT()
 * Gets edit control text from dialog
 *===========================================================================*/
...
/*=============================================================================
 * HWG_CHECKDLGBUTTON()
 * Checks or unchecks a dialog button
 *===========================================================================*/
HB_FUNC( HWG_CHECKDLGBUTTON )
{
   CheckDlgButton( ( HWND ) HB_PARHANDLE( 1 ), hb_parni( 2 ),
                   ( hb_parl( 3 ) ) ? BST_CHECKED : BST_UNCHECKED );
}

/*=============================================================================
 * HWG_CHECKRADIOBUTTON()
 * Checks a radio button in a group
 *===========================================================================*/
HB_FUNC( HWG_CHECKRADIOBUTTON )
{
   CheckRadioButton( ( HWND ) HB_PARHANDLE( 1 ), hb_parni( 2 ),
                     hb_parni( 3 ), hb_parni( 4 ) );
}

/*=============================================================================
 * HWG_ISDLGBUTTONCHECKED()
 * Checks if a dialog button is checked
 *===========================================================================*/
HB_FUNC( HWG_ISDLGBUTTONCHECKED )
{
   UINT nRes = IsDlgButtonChecked( ( HWND ) HB_PARHANDLE( 1 ), hb_parni( 2 ) );
 ...
 * Inserts a string into a combobox
 *===========================================================================*/
...
/*=============================================================================
 * HWG_COMBOSETSTRING()
 * Sets the current selection in a combobox
 *===========================================================================*/
...
/*=============================================================================
 * HWG_GETNOTIFYCODE()
 * Gets notification code from NMHDR
 *===========================================================================*/
...
/*=============================================================================
 * s_lpwAlign()
 * Helper: Aligns pointer to DWORD (4-byte) boundary
 * Required for Windows 16-bit DLGTEMPLATE format
 *===========================================================================*/
...
/*=============================================================================
 * s_nCopyAnsiToWideChar()
 * Copies ANSI string to wide char buffer
 *===========================================================================*/
...
/*=============================================================================
 * s_nWideStringLen()
 * Returns the length of a wide string (including null terminator)
 *===========================================================================*/
...
/*=============================================================================
 * HWG_MAX_INLINE_CAPTION
 * Maximum caption length kept inline in DLGTEMPLATE (16-bit format)
 * Captions larger than this are applied via WM_SETTEXT after dialog creation
 *===========================================================================*/
#define HWG_MAX_INLINE_CAPTION   2000

/*=============================================================================
 * s_CreateDlgTemplate()
 * Creates a Windows DLGTEMPLATE structure from a Harbour object
 *===========================================================================*/
                                          ...
                                          int dwidth, int dheight, ULONG ulStyle )
{
   ...
   if( !hgbl )
   {
      if( pControls ) hb_itemRelease( pControls );
      return NULL;
   }
   ...
   *p++ = 0;                    /* Class */
   ...
   if( pControls && HB_IS_ARRAY( pControls ) )
   {
      for( ul = 1; ul <= ulControls; ul++ )
      {
         ...
         *p++ = dwidth;
         *p++ = dheight;
         *p++ = hb_itemGetNI( GetObjectVar( pControl, "ID" ) );
         *p++ = 0;
         ...
         /* Caption - too large? leave it empty, applied via WM_SETTEXT later */
         {
...
/*=============================================================================
 * s_ReleaseDlgTemplate()
 * Releases a DLGTEMPLATE structure
 *===========================================================================*/
...
/*=============================================================================
 * s_FixupLargeCaptions()
 * Applies captions that were too large for the 16-bit DLGTEMPLATE
 * Called from WM_INITDIALOG after controls exist
 *===========================================================================*/
static void s_FixupLargeCaptions( HWND hDlg, PHB_ITEM pObj )
{
   ...
   for( ul = 1; ul <= ulControls; ul++ )
   {
      ...
      PHB_ITEM pTitle   = hb_itemNew( GetObjectVar( pControl, "TITLE" ) );

      if( pTitle && HB_IS_STRING( pTitle ) &&
            s_nWideStringLen( pTitle ) > HWG_MAX_INLINE_CAPTION )
      {
         int  nID  = hb_itemGetNI( GetObjectVar( pControl, "ID" ) );
      ...
      if( pTitle ) hb_itemRelease( pTitle );
   }
...
/*=============================================================================
 * HWG__PROPERTYSHEET()
 * Creates a property sheet dialog
 *===========================================================================*/
 ...
 * Creates a dialog from a template
 *===========================================================================*/
HB_FUNC( HWG_CREATEDLGINDIRECT )
{
   ...
   {
      ULONG ulStyle = ( ( hb_pcount(  ) > 6 && !HB_ISNIL( 7 ) ) ? ( ULONG ) hb_parnl( 7 ) : WS_POPUP | WS_VISIBLE | WS_CAPTION | WS_SYSMENU | WS_SIZEBOX );
...
/*=============================================================================
 * HWG_DLGBOXINDIRECT()
 * Displays a modal dialog from a template
 *===========================================================================*/
HB_FUNC( HWG_DLGBOXINDIRECT )
{
   PHB_ITEM pObject = hb_param( 2, HB_IT_OBJECT );
   ULONG ulStyle = ( ( hb_pcount(  ) > 6 && !HB_ISNIL( 7 ) ) ? ( ULONG ) hb_parnl( 7 ) : WS_POPUP | WS_VISIBLE | WS_CAPTION | WS_SYSMENU );
...
/*=============================================================================
 * s_PSPProcRelease()
 * Property sheet page callback for release
 *===========================================================================*/
...
/*=============================================================================
 * HWG_GETNOTIFYIDFROM()
 * Gets notification ID from NMHDR
 *===========================================================================*/

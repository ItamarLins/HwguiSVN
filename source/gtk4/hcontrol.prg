/*
 *$Id: hcontrol.prg 3799 2025-12-01 08:22:50Z alkresin $
 *
 ...
 *
 *   - hwg_Createstatuswindow / hwg_Writestatuswindow  → GtkBox + GtkLabel
 *   - hwg_Createstatic                                 → GtkLabel
 *   - hwg_Createbutton (HButton / HGroup)              → GtkButton / GtkFrame
 *   - hwg_CreateSep (HLine)                            → GtkSeparator
 *   - hwg_CreateBoard (HBoard)                         → GtkDrawingArea
 *   - hwg_Beginpaint / hwg_Endpaint                    → bridge to live cairo_t
 *   - hwg_SetSignal("clicked") on HButton              → unchanged (GtkButton
 *                                                         still has "clicked")
 *
 * The WM_GETDLGCODE branch in HBoard:onEvent is dead code on GTK — it was
 * already inert in GTK2 and remains so in GTK4.
 *
 * The only external dependency that still needs to be ported is
 * hwg_ShowAll / hwg_HideHidden (see wintools.c):
 *     gtk_widget_show_all()   → gtk_widget_set_visible(w, TRUE)
 */

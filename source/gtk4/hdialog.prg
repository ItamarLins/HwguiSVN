/*
 *$Id: hdialog.prg 3852 2026-08-16 20:42:12Z itamarlins $
 *
 ...
 * NOTE
 * ----
 * This file needs no changes for GTK4.  The signal names it relies on
 * are translated to their GTK4 counterparts inside set_signal()
 * (window.c):
 *
 *     "map-event" on any widget   →  notify::mapped
 *
 * And the wrapper HWG_SHOWALL is expected to be adapted in the C side
 * (wintools.c) to call gtk_widget_set_visible(w, TRUE) — GtkWidget::
 * show_all was removed in GTK4.
 */
...
STATIC FUNCTION onDestroy( oDlg )
   ...
   /*
    * REMOVED: hwg_gtk_exit() was quitting s_MainLoop (the application
    * loop), taking the whole system down whenever a modal dialog
    * closed.  In GTK4 the dialog's nested GMainLoop is already
    * terminated in two places:
    *
    *   - cb_close_request()      (window.c) -- user clicks the X
    *   - HWG_DESTROYWINDOW()     (window.c) -- called from hwg_EndDialog
    *                                          right after this function
    *
    * hwg_gtk_exit() must only be used to shut down the application
    * itself, never as part of a dialog teardown.
    */
...
CLASS HDialog INHERIT HWindow
   ...
   DATA lExitOnEsc   INIT .T. // Set it to False, if dialog shouldn't be ended after pressing ESC key
   DATA oIcon, oBmp
   DATA xResourceID
   DATA lModal
...
METHOD Activate( lNoModal, lMaximized, lMinimized, lCentered, bActivate ) CLASS HDialog
   ...
   /* GTK4: "map-event" was removed.  set_signal() translates it to
      notify::mapped so ::Center() still runs when the window is
      actually shown by the compositor. */

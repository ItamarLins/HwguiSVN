/*
 * $Id: hprogres.prg 3750 2025-05-09 20:33:12Z itamarlins $
 *
 ...
 *
 *   - hwg_Createprogressbar  → GtkProgressBar        (control.c)
 *   - hwg_Updateprogressbar  → gtk_progress_bar_pulse
 *   - hwg_Setprogressbar     → gtk_progress_bar_set_fraction
 *   - hwg_Resetprogressbar   → set_fraction(0.0)
 *   - hwg_Setwindowtext      → gtk_window_set_title  (window.c)
 *   - hwg_EndDialog          → gtk_window_destroy    (hdialog.prg)
 *
 * The Ticket #52 guard (0 <= nPos <= 1) is still required — GTK4's
 * gtk_progress_bar_set_fraction() also asserts on out-of-range values.
 */

/*
 *$Id: hprinter.prg 3796 2025-08-27 16:19:05Z df7be $
 *
 ...
 *
 *   - Print pipeline (hwg_OpenPrinter, hwg_gp_Print, hwg_gp_GetDeviceArea,
 *     hwg_Setprintermode, ...)   → wprint.c  (already GTK4)
 *
 *   - Cairo rendering (hwg_Definepaintstru, hwg_Beginpaint, hwg_Endpaint,
 *     hwg_Fillrect, hwg_Drawbitmap, hwg_cairo_translate, ...) → draw.c
 *
 *   - Preview dialog is just a normal HPanel + HOwnButton + HComboBox
 *     composition, all of which are already GTK4-ready.
 *
 * The only external dependency worth flagging:
 *
 *   - hwg_Getdevicearea() with no arguments (physical screen size for
 *     the SCREEN_PRINTER buffer mode) lives in wintools.c and still has
 *     to be ported to GTK4 — see the GdkMonitor-based snippet.
 */
...
FUNCTION hwg_HPrinter_LangArray_EN()
  ...
  /* 10 */ AAdd(aTooltips,"Print preview -")
  /* 11 */ AAdd(aTooltips,"Print")
  /* 12 */ AAdd(aTooltips,"Exit")
  /* 13 */ AAdd(aTooltips,"Dialog")
  /* 14 */ AAdd(aTooltips,"User Button")
  /* 15 */ AAdd(aTooltips,"User Button")
  /* 16 */ AAdd(aTooltips,"All")
  /* 17 */ AAdd(aTooltips,"Current")
  /* 18 */ AAdd(aTooltips,"Pages")
  /* 19 */ AAdd(aTooltips,"Print")
  /* 20 */ AAdd(aTooltips,"Cancel")
  /* 21 */ AAdd(aTooltips,"Enter range of pages")
...
METHOD Preview( cTitle, aBitmaps, aTooltips, aBootUser ) CLASS HPrinter
   ...
   @ 3, 2 OWNERBUTTON oBtn ON CLICK { || hwg_EndDialog() } ;
      SIZE TOOL_SIDE_WIDTH - 6, 24 TEXT cmExit FONT oFont  ;
  ...
  IF ::lprbutton
   @ 3, 36 OWNERBUTTON oBtn  ON CLICK { || ::PrintDoc() } ;
      SIZE TOOL_SIDE_WIDTH - 6, 24 TEXT cmPrint FONT oFont         ;

...
/* -----------------------------------------------------------------
 * TAB disabled support (GTK) - prevents click on disabled tab label
 * and allows Harbour side to toggle enabled/disabled.
 * -----------------------------------------------------------------*/
...
static GArray * hwg_tab_disabled_get( GtkNotebook *nb )
{
      ...
      if( !arr )
      {
            ...
            g_object_set_data_full( (GObject*) nb, "hwg_tab_disabled", arr, hwg_tab_disabled_free );
      }
      return arr;
}
...
static gboolean cb_tablabel_button_press( GtkWidget *w, GdkEventButton *event, gpointer user_data )
{
      GtkNotebook *nb = (GtkNotebook*) user_data;
      guint idx = (guint) GPOINTER_TO_UINT( g_object_get_data( (GObject*) w, "hwg_tab_idx" ) );
      ...
      if( nb )
      {
            ...
            if( arr && idx < arr->len )
            {
                  gboolean disabled = g_array_index( arr, gboolean, idx );
                  if( disabled )
                        return TRUE; /* stop click */
            }
      }
      return FALSE; /* allow normal */
}
                    ...
                    guint page_num, gpointer user_data )
{
      ...
      /*
       * CRITICAL VALIDATION (GTK): Check if the targeted tab index is locked.
       * If disabled, we immediately intercept the runtime engine state to abort processing.
       */
      if( arr && page_num < arr->len )
      {
            ...
            if( disabled )
            {
                  /* Stop signal propagation and halt the native GTK tab transition */
...
HB_FUNC( HWG_ADDTAB )
{
      ...
      /* put label inside an event box so we can block clicks when disabled */
      hEvtBox = gtk_event_box_new();
      gtk_event_box_set_visible_window( GTK_EVENT_BOX( hEvtBox ), FALSE );
      gtk_container_add( GTK_CONTAINER( hEvtBox ), hLabel );
      gtk_widget_show( hLabel );

      /* ensure disabled array size and store current index (0-based) */
      nIdx = (guint) gtk_notebook_get_n_pages( nb );
      hwg_tab_disabled_ensure( nb, nIdx + 1 );
      g_object_set_data( (GObject*) hEvtBox, "hwg_tab_idx", GUINT_TO_POINTER( nIdx ) );
      gtk_widget_add_events( hEvtBox, GDK_BUTTON_PRESS_MASK );
      g_signal_connect( G_OBJECT( hEvtBox ), "button-press-event",
                        G_CALLBACK( cb_tablabel_button_press ), (gpointer) nb );

      gtk_notebook_append_page( nb, box, hEvtBox );
...
HB_FUNC( HWG_SETTABDISABLED )
{
      ...
      /* also set widget sensitivity, so it behaves visually like disabled */
...
HB_FUNC( HWG_ADDTOOLTIP )
{
      ...
      if (gcTitle && GTK_IS_WIDGET(widget)) {
